...
# =============================================================================
# ENDPOINT TEMPORANEO PER MIGRATION DATABASE (RIMUOVERE DOPO L'USO)
# =============================================================================

@app.get("/admin/migrate-problemi-fields")
async def migrate_problemi_fields(db: Session = Depends(get_db)):
    """Migration temporanea per aggiungere campi problemi - RIMUOVERE DOPO L'USO"""
    try:
        from sqlalchemy import text
        
        # Aggiungi le nuove colonne
        migrations = [
            "ALTER TABLE acquisti ADD COLUMN IF NOT EXISTS problema_tipo VARCHAR(50)",
            "ALTER TABLE acquisti ADD COLUMN IF NOT EXISTS problema_descrizione TEXT", 
            "ALTER TABLE acquisti ADD COLUMN IF NOT EXISTS problema_data_segnalazione TIMESTAMP",
            "ALTER TABLE acquisti ADD COLUMN IF NOT EXISTS problema_segnalato BOOLEAN DEFAULT FALSE NOT NULL"
        ]
        ...
        for sql in migrations:
            try:
                ...
                results.append(f"✅ {sql} - SUCCESS")
            except Exception as e:
                if "already exists" in str(e).lower():
                    results.append(f"ℹ️ {sql} - ALREADY EXISTS")
                else:
                    results.append(f"❌ {sql} - ERROR: {str(e)}")
        
        # Aggiorna record esistenti
        update_result = db.execute(text(
            "UPDATE acquisti SET problema_segnalato = FALSE WHERE problema_segnalato IS NULL"
        ))
        results.append(f"✅ Updated {update_result.rowcount} existing records")
        
        # Verifica colonne create
        verify_result = db.execute(text("""
            SELECT column_name, data_type, is_nullable 
            FROM information_schema.columns 
            WHERE table_name = 'acquisti' 
            AND column_name LIKE 'problema%'
        """))
        
        columns = [dict(row) for row in verify_result.fetchall()]
        ...
        return {
            ...
            "message": "✅ MIGRATION COMPLETED! Remember to remove this endpoint and update models.py"
        }
...
# =============================================================================
# NUOVE ROUTE PER GESTIONE PROBLEMI (MANTENERE PERMANENTI)
# =============================================================================

@app.get("/acquisti-non-arrivati", response_class=HTMLResponse)
async def acquisti_non_arrivati_page(
    request: Request, 
    db: Session = Depends(get_db),
    filtro_stato: str = "tutti",
    ordinamento: str = "attesa_desc",
    cerca: str = ""
):
    """Pagina dedicata agli acquisti non ancora arrivati con gestione problemi"""
    
    # Query base: tutti gli acquisti senza data_consegna
    query = db.query(Acquisto).filter(Acquisto.data_consegna.is_(None))
    
    # Calcolo dei giorni di attesa per ogni acquisto
    ...
    if filtro_stato == "normali":
        # Acquisti con attesa <= 7 giorni e senza problemi segnalati
        query = query.filter(
            and_(
                or_(
                    func.extract('days', oggi - Acquisto.data_pagamento) <= 7,
                    Acquisto.data_pagamento.is_(None)
                ),
                # Controllo campo problema_segnalato solo se esiste
                or_(
                    Acquisto.problema_segnalato == False,
                    Acquisto.problema_segnalato.is_(None)
                )
            )
        )
    elif filtro_stato == "ritardo":
        # Acquisti con attesa > 7 giorni
        query = query.filter(
            func.extract('days', oggi - Acquisto.data_pagamento) > 7
        )
    elif filtro_stato == "critico":
        # Acquisti con attesa > 14 giorni
        query = query.filter(
            func.extract('days', oggi - Acquisto.data_pagamento) > 14
        )
    elif filtro_stato == "con_problemi":
        # Solo acquisti con problemi segnalati (se il campo esiste)
        try:
            query = query.filter(Acquisto.problema_segnalato == True)
        except AttributeError:
            # Se il campo non esiste ancora, nessun filtro
            pass
    elif filtro_stato == "senza_problemi":
        # Acquisti senza problemi segnalati (se il campo esiste)
        try:
            query = query.filter(
                or_(
                    Acquisto.problema_segnalato == False,
            ...
            # Se il campo non esiste ancora, tutti vanno bene
            pass
    
    # Ordinamento
    if ordinamento == "attesa_desc":
        query = query.order_by(Acquisto.data_pagamento.asc().nulls_last())
    elif ordinamento == "attesa_asc":
        query = query.order_by(Acquisto.data_pagamento.desc().nulls_first())
    elif ordinamento == "data_desc":
        query = query.order_by(Acquisto.data_pagamento.desc().nulls_last())
    elif ordinamento == "data_asc":
        query = query.order_by(Acquisto.data_pagamento.asc().nulls_first())
    elif ordinamento == "costo_desc":
        query = query.order_by(Acquisto.costo_acquisto.desc())
    elif ordinamento == "problemi_desc":
        # Ordinamento per problemi (se il campo esiste)
        try:
            query = query.order_by(Acquisto.problema_segnalato.desc(), Acquisto.data_pagamento.asc())
        except AttributeError:
            query = query.order_by(Acquisto.data_pagamento.asc().nulls_last())
    
    acquisti_non_arrivati = query.options(joinedload(Acquisto.prodotti)).all()
    
    # Calcolo giorni di attesa e classificazione
    for acquisto in acquisti_non_arrivati:
        if acquisto.data_pagamento:
            acquisto.giorni_attesa = (oggi - acquisto.data_pagamento).days
        else:
            acquisto.giorni_attesa = (oggi - acquisto.created_at.date()).days if acquisto.created_at else 0
        
        # Gestisci campi problema (potrebbero non esistere ancora)
        if not hasattr(acquisto, 'problema_segnalato'):
            acquisto.problema_segnalato = False
        if not hasattr(acquisto, 'problema_tipo'):
            acquisto.problema_tipo = None
        if not hasattr(acquisto, 'problema_descrizione'):
            acquisto.problema_descrizione = None
    
    # Classificazione per statistiche (gestisce campi mancanti)
    acquisti_normali = []
    acquisti_ritardo = []
    acquisti_con_problemi = []
    
    for a in acquisti_non_arrivati:
        problema_segnalato = getattr(a, 'problema_segnalato', False)
        
        if a.giorni_attesa <= 7 and not problema_segnalato:
            acquisti_normali.append(a)
        elif a.giorni_attesa > 7:
            acquisti_ritardo.append(a)
        
        if problema_segnalato:
            acquisti_con_problemi.append(a)
    
    # Calcoli statistiche
    investimento_totale = sum(a.costo_totale for a in acquisti_non_arrivati)
    prodotti_totali = sum(a.numero_prodotti for a in acquisti_non_arrivati)
    
    return templates.TemplateResponse("acquisti_non_arrivati.html", {
        "request": request,
        "acquisti_non_arrivati": acquisti_non_arrivati,
        "acquisti_filtered": acquisti_non_arrivati,  # Per compatibilità template
        "acquisti_normali": acquisti_normali,
        "acquisti_ritardo": acquisti_ritardo,
        "acquisti_con_problemi": acquisti_con_problemi,
...
@app.post("/acquisti/segnala-problema")
async def segnala_problema_acquisto(request: Request, db: Session = Depends(get_db)):
    """API per segnalare un problema su un acquisto"""
    try:
        body = await request.json()
        acquisto_id = body.get("acquisto_id")
        problema_tipo = body.get("problema_tipo")
        problema_descrizione = body.get("problema_descrizione", "")
        
        if not acquisto_id or not problema_tipo:
            return {"success": False, "error": "Parametri mancanti"}
        ...
        # Verifica che i campi problema esistano (potrebbero non essere ancora migrati)
        try:
            # Aggiorna l'acquisto con il problema
            acquisto.problema_tipo = problema_tipo
            acquisto.problema_descrizione = problema_descrizione
            acquisto.problema_data_segnalazione = datetime.now()
            acquisto.problema_segnalato = True
            
            db.commit()
            
            return {
                "success": True,
                "message": f"Problema '{problema_tipo.replace('_', ' ').title()}' segnalato per l'acquisto {acquisto.id_acquisto_univoco}"
            }
        except AttributeError as e:
            return {
                "success": False, 
                "error": "Campi problema non ancora disponibili. Esegui prima la migration database."
            }
        
    except Exception as e:
        db.rollback()
        print(f"Errore segnalazione problema: {str(e)}")
        return {"success": False, "error": f"Errore interno: {str(e)}"}


@app.post("/acquisti/risolvi-problema")
async def risolvi_problema_acquisto(request: Request, db: Session = Depends(get_db)):
    """API per risolvere/rimuovere un problema da un acquisto"""
    try:
        body = await request.json()
        acquisto_id = body.get("acquisto_id")
        
        if not acquisto_id:
            return {"success": False, "error": "ID acquisto mancante"}
        ...
        # Verifica che i campi problema esistano
        try:
            # Rimuovi il problema
            acquisto.problema_tipo = None
            acquisto.problema_descrizione = None
            acquisto.problema_data_segnalazione = None
            acquisto.problema_segnalato = False
            ...
            return {
                "success": True,
                "message": f"Problema risolto per l'acquisto {acquisto.id_acquisto_univoco}"
            }
        except AttributeError as e:
            return {
                "success": False, 
        ...
        db.rollback()
        print(f"Errore risoluzione problema: {str(e)}")
        return {"success": False, "error": f"Errore interno: {str(e)}"}


@app.post("/acquisti-non-arrivati/segna-tutti-arrivati")
async def segna_tutti_non_arrivati_come_arrivati(request: Request, db: Session = Depends(get_db)):
    """API per segnare tutti gli acquisti non arrivati come arrivati oggi"""
    try:
        oggi = datetime.now().date()
        
        # Trova tutti gli acquisti non arrivati
        acquisti_non_arrivati = db.query(Acquisto).filter(
            Acquisto.data_consegna.is_(None)
        ).all()
        ...
        return {
            "success": True,
            "message": f"{count_updated} acquisti segnati come arrivati oggi"
        }
        
    except Exception as e:
        db.rollback()
        print(f"Errore aggiornamento multiplo: {str(e)}")
        return {"success": False, "error": f"Errore interno: {str(e)}"}
