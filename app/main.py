...
# NUOVE API per gestione problemi
...
async def segnala_problema_acquisto(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        acquisto_id = data.get("acquisto_id")
        problema_tipo = data.get("problema_tipo")
        problema_descrizione = data.get("problema_descrizione", "")
        
        if not all([acquisto_id, problema_tipo]):
            return {"success": False, "error": "Parametri mancanti"}
        ...
        # Aggiorna i campi problema
        acquisto.problema_segnalato = True
        acquisto.problema_tipo = problema_tipo
        acquisto.problema_descrizione = problema_descrizione
        acquisto.problema_data_segnalazione = date.today()
        
        db.commit()
        
        return {
            "success": True,
            "message": f"Problema '{problema_tipo}' segnalato per acquisto {acquisto.id_acquisto_univoco}"
        }
...
async def risolvi_problema_acquisto(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Reset dei campi problema
        problema_precedente = acquisto.problema_tipo
        ...
        return {
            "success": True,
            "message": f"Problema '{problema_precedente}' risolto per acquisto {acquisto.id_acquisto_univoco}"
        }
