...
# QUESTO E' IL TUO main.py ATTUALE + solo l'endpoint di migration sicura
# NON sostituire tutto, AGGIUNGI solo l'endpoint alla fine prima di "if __name__"

# ... tutto il tuo codice main.py attuale rimane uguale ...
# AGGIUNGI solo questo endpoint PRIMA della riga: if __name__ == "__main__":

@app.get("/admin/safe-migrate-problemi")
async def safe_migrate_problemi(db: Session = Depends(get_db)):
    """Migration sicura - aggiunge solo i campi DB senza toccare models"""
    try:
        from sqlalchemy import text
        ...
        columns_to_add = [
            ("problema_tipo", "VARCHAR(50)"),
            ("problema_descrizione", "TEXT"), 
            ("problema_data_segnalazione", "TIMESTAMP"),
        ...
        results.append(f"📋 Colonne esistenti: {list(existing_columns)}")
        
        # Aggiungi solo le colonne che non esistono
        columns_added = 0
        for column_name, column_type in columns_to_add:
            if column_name not in existing_columns:
                try:
                    sql = f"ALTER TABLE acquisti ADD COLUMN {column_name} {column_type}"
                    db.execute(text(sql))
                    results.append(f"✅ Aggiunta colonna: {column_name}")
                    columns_added += 1
                except Exception as e:
                    results.append(f"❌ Errore {column_name}: {str(e)}")
            else:
                results.append(f"ℹ️ Colonna già esistente: {column_name}")
        
        # Aggiorna i record esistenti per il campo boolean
        if "problema_segnalato" not in existing_columns or columns_added > 0:
            try:
                update_result = db.execute(text(
                    "UPDATE acquisti SET problema_segnalato = FALSE WHERE problema_segnalato IS NULL"
                ))
                results.append(f"✅ Aggiornati {update_result.rowcount} record esistenti")
            except Exception as e:
                results.append(f"⚠️ Errore aggiornamento: {str(e)}")
        ...
        return {
            ...
            "migration_log": results,
            "final_columns": final_columns,
            "message": f"✅ Migration completata! {columns_added} nuove colonne aggiunte. Ora puoi aggiornare models.py"
        }
        ...
        return {
            "success": False,
            "error": str(e),
            "message": "❌ Migration fallita"
        }

# ... il resto del tuo main.py rimane uguale ...
